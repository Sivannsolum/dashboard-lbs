    ...
    <v-dialog v-model="show" width="700">
      <v-card class="popup add_store_popup">
        ...
        <p class="label-txt">
          ...
          <span class="ml-5"
            ><strong>**</strong> {{$t('Gateways will be rebooted by changing the config')}}</span
          >
        </p>

        <v-card-actions class="d-flex justify-center">
          <v-btn text icon @click="updateSingleGateway" class="btn">{{$t('Save')}}</v-btn>
          <v-btn text icon @click="show = false" class="btn">{{$t('Cancel')}}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
  ...
  methods: {
    ...
    updateSingleGateway: function () {
      ...
      this.$utils
        ...
        .then(res => {
          EventBus.$emit('messageAlert', this.$t('The gateway information has been updated successfully'))
          this.$emit('fireUpdateGatewayList')
        })
        .catch(error => {
          EventBus.$emit('messageAlert', this.$t('Failed to updated the Gateway'))
          console.log(error)
        })
    }
  }
