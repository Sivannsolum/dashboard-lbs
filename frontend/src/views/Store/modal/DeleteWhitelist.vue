<template>
    <v-dialog
        ...
        width="400"
        @click:outside="cancelDeleteWhitelist"
    >
        <template v-slot:activator="{ on, attrs }">
            <v-btn :disabled="deleteWhitelistDisabled" text v-bind="attrs" v-on="on" class="btn-s ml-2">{{$t('Delete')}}</v-btn>
        </template>

        <v-card class="popup add_store_popup">
        <h3 class="page-title-bar"><i class="ico ico-store"></i>{{ $t('Delete Whitelist') }}</h3>
        <v-row>
            <v-col>
        ...
        <v-card-actions class="d-flex justify-center">
            <v-btn @click="deleteWhitelist" text icon class="btn">{{$t('Delete')}}</v-btn>
            <v-btn text icon @click="cancelDeleteWhitelist" class="btn">{{$t('Cancel')}}</v-btn>
        </v-card-actions>
        </v-card>
    </v-dialog>
</template>
