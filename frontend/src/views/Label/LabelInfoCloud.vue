  ...
  <div>
    ...
    >
      ...
      <template v-slot:[`item.history`]="{ item }">
        <label-history :item="item"> </label-history>
      </template>
      <!-- end : history 추가 -->
    </v-data-table>
    <!-- Label detail popup -->
      ...
      <div>
        ...
        <div class="pagination2">
          ...
          <button
            ...
            class="pagination-nav pagination-nav-next pagination-nav-disapled"
            type="button"
          >
            <img
...
import popupLabelCloud from './modal/LabelPopupLabelCloud'
import BlinkPopup from './modal/LabelBlinkPopup'
import LabelHistory from './modal/LabelHistory.vue'
  ...
  computed: {
    labelHeaders () {
      return [
        ...
        { text: this.$t('TYPE'), sortable: false, value: 'type', width: '15%' },
        {
          text: this.$t('NETWORK'),
          sortable: false,
          value: 'network',
        ...
        {
          text: this.$t('LATEST RESPONSE TIME'),
          sortable: true,
          value: 'time'
        }, // 09.24 텍스트 수정
        {
  ...
  watch: {
    ...
    searchBy: {
      handler (newSearchBy) {
        ...
        if (newSearchBy.statusParam) {
          newSearchBy.gatewayParam = null
          newSearchBy.typeParam = null
          newSearchBy.templateParam = null

          this.searchParam.network = null
          this.searchParam.battery = null
          this.searchParam.signal = null
          this.searchParam.misusedLabelType = null
    ...
    // pageCount () {
    //   this.pagenationHandler = false
    //   setTimeout(() => {
    //     this.pagenationHandler = true
    //   }, 1000)
    // }
  },
  mounted () {
  ...
  methods: {
    ...
    openLabelModal (item) {
      ...
      if (this.productionType === 'cloud') {
        EventBus.$emit('getLabelDetailPopupDataValuesCloud', item)
        EventBus.$emit('openLabelDetailCloud')
      } else {
